...
-- Parâmetro data_referencia: permite gerar snapshot das consultas emergenciais em uma data específica
CREATE OR REPLACE PROCEDURE `rj-sms-sandbox.sub_pav_us.proced_4_consultas_emergenciais_historico`(data_referencia DATE)
...
--     config(
--         enabled=true,
--         alias="consultas_emergenciais_historico",
--     )
-- }}

CREATE OR REPLACE TABLE `rj-sms-sandbox.sub_pav_us._consultas_emergenciais_historico` AS
...
--  FROM {{ ref('mart_historico_clinico__episodio') }} ea
 ...
 AND ea.entrada_data BETWEEN mt.data_inicio AND COALESCE(mt.data_fim_efetiva, data_referencia)  -- ALTERADO: era CURRENT_DATE()
