...
-- Parâmetro data_referencia: permite gerar snapshot dos encaminhamentos SISREG em uma data específica
CREATE OR REPLACE PROCEDURE `rj-sms-sandbox.sub_pav_us.proced_5_encaminhamentos_historico`(data_referencia DATE)
...
--     config(
--         enabled=true,
--         alias="encaminhamentos_historico",
--     )
-- }}

CREATE OR REPLACE TABLE `rj-sms-sandbox.sub_pav_us._encaminhamentos_historico` AS
 ...
 SELECT
   *,
   CASE
     ...
     WHEN DATE_ADD(data_inicio, INTERVAL 308 DAY) <= data_referencia THEN DATE_ADD(data_inicio, INTERVAL 308 DAY) -- ALTERADO: era CURRENT_DATE()
...
filtrado AS (
 SELECT
   gcs.*,
   CASE
     WHEN gcs.data_fim IS NULL AND DATE_ADD(gcs.data_inicio, INTERVAL 308 DAY) > data_referencia THEN 'Gestação' -- ALTERADO: era CURRENT_DATE()
     WHEN gcs.data_fim IS NOT NULL AND DATE_DIFF(data_referencia, gcs.data_fim, DAY) <= 45 THEN 'Puerpério' -- ALTERADO: era CURRENT_DATE()
   ...
   CASE
     WHEN DATE_DIFF(data_referencia, gcs.data_inicio, WEEK) <= 13 THEN '1º trimestre' -- ALTERADO: era CURRENT_DATE()
     WHEN DATE_DIFF(data_referencia, gcs.data_inicio, WEEK) BETWEEN 14 AND 27 THEN '2º trimestre' -- ALTERADO: era CURRENT_DATE()
     WHEN DATE_DIFF(data_referencia, gcs.data_inicio, WEEK) >= 28 THEN '3º trimestre' -- ALTERADO: era CURRENT_DATE()
...
pacientes_info AS (
 SELECT
   ...
   DATE_DIFF(data_referencia, p_dedup.dados.data_nascimento, YEAR) AS idade_atual, -- ALTERADO: era CURRENT_DATE()
   CASE
     WHEN DATE_DIFF(data_referencia, p_dedup.dados.data_nascimento, YEAR) <= 15 THEN '≤15 anos' -- ALTERADO: era CURRENT_DATE()
     WHEN DATE_DIFF(data_referencia, p_dedup.dados.data_nascimento, YEAR) <= 20 THEN '16-20 anos' -- ALTERADO: era CURRENT_DATE()
     WHEN DATE_DIFF(data_referencia, p_dedup.dados.data_nascimento, YEAR) <= 30 THEN '21-30 anos' -- ALTERADO: era CURRENT_DATE()
     WHEN DATE_DIFF(data_referencia, p_dedup.dados.data_nascimento, YEAR) <= 40 THEN '31-40 anos' -- ALTERADO: era CURRENT_DATE()
     ELSE '>40 anos'
   END AS faixa_etaria,
   p_dedup.dados.raca,
...
encaminhamento_SISREG AS (
 ...
 LEFT JOIN
  --  {{ ref('raw_sisreg_api__solicitacoes') }} s
   rj-sms.brutos_sisreg_api.solicitacoes s
   ON
     (
     ...
     AND DATE(s.data_solicitacao) BETWEEN f.data_inicio AND COALESCE(f.data_fim_efetiva, data_referencia)  -- ALTERADO: era CURRENT_DATE()
)
