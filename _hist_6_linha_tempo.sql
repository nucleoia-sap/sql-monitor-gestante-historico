...
partos_associados AS (
 ...
 JOIN eventos_parto e
   ...
   AND e.data_parto BETWEEN f.data_inicio AND DATE_ADD(COALESCE(f.data_fim_efetiva, f.dpp, data_referencia), INTERVAL 15 DAY)  -- ALTERADO: era CURRENT_DATE()
...
visitas_acs_por_gestacao AS (
    ...
    FROM
        ...
        `rj-sms-sandbox.sub_pav_us._visitas_acs_gestacao_historico`
    WHERE data_snapshot = data_referencia  -- NOVO: Filtrar pelo snapshot específico
    GROUP BY
        id_gestacao
),
...
maior_pa_por_gestacao AS (
    ...
    FROM (
            ...
            FROM
                ...
                `rj-sms-sandbox.sub_pav_us._atendimentos_prenatal_aps_historico`
            WHERE
                data_snapshot = data_referencia  -- NOVO: Filtrar pelo snapshot específico
...
analise_pressao_arterial AS (
    ...
    FROM
        ...
        AND fapn.data_snapshot = data_referencia  -- NOVO: Filtrar pelo snapshot específico
...
Urgencia_e_emergencia as (
    ...
    FROM ( -- Adicionado subquery para aplicar ROW_NUMBER e filtrar
            ...
            FROM
                ...
                LEFT JOIN `rj-sms-sandbox.sub_pav_us._consultas_emergenciais_historico` fapn
                    ON f.id_gestacao = fapn.id_gestacao
                    AND fapn.data_snapshot = data_referencia  -- NOVO: Filtrar pelo snapshot específico
                ...
                AND DATE(fapn.data_consulta) BETWEEN f.data_inicio AND COALESCE(
                    f.data_fim_efetiva, f.dpp, data_referencia  -- ALTERADO: era CURRENT_DATE()
                )
            WHERE
