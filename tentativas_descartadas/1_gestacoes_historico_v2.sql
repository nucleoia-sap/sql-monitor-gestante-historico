...
-- VERSÃO PARAMETRIZADA PARA CONSTRUÇÃO DE HISTÓRICO - V2
-- CORREÇÃO: Agrupamento temporal por data_atendimento, não por DUM
-- Parâmetro data_referencia: permite gerar snapshot dos dados em uma data específica
CREATE OR REPLACE PROCEDURE `rj-sms-sandbox.sub_pav_us.proced_1_gestacoes_historico_v2`(data_referencia DATE)
...
--     config(
--         enabled=true,
--         alias="gestacoes_historico_v2",
--     )
-- }}

CREATE OR REPLACE TABLE `rj-sms-sandbox.sub_pav_us._gestacoes_historico_v2` AS
...
WITH
    ...
    eventos_brutos AS (
        SELECT
            ...
            entrada_data AS data_atendimento,  -- ✅ Data do atendimento (para agrupamento temporal)
            SAFE.PARSE_DATE (
                '%Y-%m-%d',
                SUBSTR(c.data_diagnostico, 1, 10)
            ) AS data_evento,  -- ✅ DUM registrada (para calcular MODA)
            CASE
                WHEN c.id = 'Z321'
                OR c.id LIKE 'Z34%'
        ...
        WHERE
            ...
            -- Filtro de janela temporal baseado na DUM
    ...
    -- ✅ CORREÇÃO: Usa data_atendimento para agrupamento, não DUM
    -- Se diferença entre ATENDIMENTOS > 60 dias, considera nova gestação
    eventos_com_periodo AS (
        SELECT
            *,
            LAG(data_atendimento) OVER (
                PARTITION BY id_paciente
                ORDER BY data_atendimento
            ) AS data_atendimento_anterior,
            CASE
                WHEN LAG(data_atendimento) OVER (
                    PARTITION BY id_paciente
                    ORDER BY data_atendimento
                ) IS NULL THEN 1
                WHEN DATE_DIFF(
    ...
    -- ✅ CORREÇÃO: Ordena por data_atendimento
    ...
    -- ------------------------------------------------------------
    -- Mesmo com nova lógica de início, mantemos detecção de fim
    -- via CIDs RESOLVIDOS
    -- ------------------------------------------------------------
    finais AS (
        SELECT *
